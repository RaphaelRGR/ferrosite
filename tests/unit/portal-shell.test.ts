...
describe("navegação do Portal", () => {
  ...
  it("só lista rotas existentes e rotuladas pelo catálogo", () => {
    expect(items.map((i) => i.href)).toEqual(["/portal", "/portal/projetos", "/portal/pessoas", "/portal/empresas", "/portal/desafios", "/portal/conteudos", "/portal/arquivos", "/portal/relatorios", "/portal/configuracoes"]);
  ...
  it("filtra por papel quando o item exige e mostra todos a contas ativas sem exigência", () => {
    ...
    expect(visibleNavItems(items, null)).toHaveLength(items.length - 4);
  });
