...
export default async function UploadPage({ searchParams }: PageProps<"/portal/arquivos/enviar">) {
  ...
  const workItemId = typeof sp.acao === "string" && /^[0-9a-f-]{36}$/.test(sp.acao) ? sp.acao : undefined;
  ...
  return (
    <div className="flex flex-col gap-8">
      <header className="border-b border-line pb-4">
        <Link href={workItemId ? `/portal/acoes/${workItemId}` : "/portal/arquivos"} className="text-xs font-bold uppercase tracking-[0.2em] text-fg-muted underline-offset-4 hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus">
          {workItemId ? dict.workItems.backToItem : f.title}
        </Link>
      ...
      <section className="rounded-xl border border-line bg-surface p-6">
        {projects.length === 0 && !overseer && !contentId && !workItemId ? (
          <p role="status" className="text-sm text-fg-muted">{f.upload.noProjects}</p>
        ) : (
          <UploadForm dict={dict} projects={projects} overseer={overseer} initialProject={initialProject} missionId={missionId} contentId={contentId} workItemId={workItemId} canWrite={Boolean(conn?.canWrite && conn.rootFolderId)} accept={types} />
        )}
      </section>
    </div>
  );
