...
export async function createProject(_prev: ActionState, fd: FormData): Promise<ActionState> {
  ...
  if (!canCreateProject(a.actor.globalRole)) return fail(fd, { error: "forbidden" });
  ...
  if (!SLUG_RE.test(slug)) return fail(fd, { error: "invalid", field: "slug" });
...
export async function updateProject(_prev: ActionState, fd: FormData): Promise<ActionState> {
  ...
  if (isState(a)) return fail(fd, a);
  if (!canManageProject(a.actor)) return fail(fd, { error: "forbidden" });
  const parsed = readProjectFields(fd);
  if (isState(parsed)) return fail(fd, parsed);
...
export async function transitionProject(_prev: ActionState, fd: FormData): Promise<ActionState> {
  ...
  if (!id || !Number.isInteger(version) || !PROJECT_STATUSES.includes(from) || !PROJECT_STATUSES.includes(to)) return fail(fd, { error: "invalid" });
  const a = await actor(id);
  if (isState(a)) return fail(fd, a);
  if (!canTransitionProject(a.actor, from, to)) return fail(fd, { error: "forbidden" });
...
export async function addMember(_prev: ActionState, fd: FormData): Promise<ActionState> {
  ...
  if (!projectId || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) return fail(fd, { error: "invalid", field: "email" });
  if (!PROJECT_ROLES.includes(role)) return fail(fd, { error: "invalid", field: "role" });
  // 11: externo expira por padrão — sem prazo, não entra.
  if (role === "external" && !expires) return fail(fd, { error: "invalid", field: "expires_at" });
  ...
  if (found.error) return fail(fd, { error: dbError(found.error) });
  const profile = found.data?.[0];
  if (!profile) return fail(fd, { error: "not_found", field: "email" });
...
export async function updateMember(_prev: ActionState, fd: FormData): Promise<ActionState> {
  ...
  const role = str(fd, "role") as (typeof PROJECT_ROLES)[number];
  const expires = dateOrNull(str(fd, "expires_at"));
  if (!projectId || !profileId || !PROJECT_ROLES.includes(role)) return fail(fd, { error: "invalid" });
  if (role === "external" && !expires) return fail(fd, { error: "invalid", field: "expires_at" });
  const a = await actor(projectId);
  if (isState(a)) return fail(fd, a);
  if (!canManageProject(a.actor)) return fail(fd, { error: "forbidden" });
...
export async function removeMember(_prev: ActionState, fd: FormData): Promise<ActionState> {
  ...
  const slug = str(fd, "slug");
  const profileId = str(fd, "profile_id");
  if (!projectId || !profileId) return fail(fd, { error: "invalid" });
  const a = await actor(projectId);
  if (isState(a)) return fail(fd, a);
  if (!canManageProject(a.actor)) return fail(fd, { error: "forbidden" });
