...
| `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `GOOGLE_REDIRECT_URI`, `DRIVE_TOKEN_KEY` | runtime (servidor) | Drive institucional por OAuth (DRIVE-002): Configurações → Integrações → Conectar. Tokens cifrados no banco (só service role). Ver `docs/GOOGLE_DRIVE_INTEGRATION.md`. |
| `GOOGLE_SERVICE_ACCOUNT_EMAIL`, `GOOGLE_SERVICE_ACCOUNT_KEY`, `GOOGLE_DRIVE_ROOT_FOLDER_ID` | runtime (servidor) | Alternativa sem OAuth (DRIVE-001), usada só quando não há conexão. `/api/health` mostra `driveConfigured` (qualquer das duas). A pasta raiz vale a do banco; a variável é só semente. |
...
| Upload falha com "somente leitura"/409 | conexão feita antes do DRIVE-003 (sem `drive.file`) | Integrações → **Reconectar para habilitar envio** (novo consentimento). |
| Upload recusado 415 | tipo não provado pelos bytes ou extensão incoerente (executável renomeado, ZIP, vídeo) | esperado; ZIP/vídeo só por registro até haver antivírus (33). |
| Pastas duplicadas no Drive | criação concorrente antes da trava (`drive_folder`) — só em rodadas antigas | o Portal não apaga; remover manualmente; a cache aponta para uma só. |
| Drive "autorização revogada" | Google invalidou o refresh token (senha, remoção do acesso, app em Teste > 7 dias). | Configurações → Integrações → **Reconectar** (admin/coordenação). Auditoria `drive.connected`. |
